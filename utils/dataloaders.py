...
class LoadImages:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            img0 = LoadImages.simplest_cb(img0, 1)                  #---------------------------------------------------------------------------------------------
            # cv2.imshow('1',img0)
            # cv2.waitKey(10000)
    ...
    def apply_threshold(matrix, low_value, high_value):
        ...
        matrix = LoadImages.apply_mask(matrix, low_mask, low_value)
        # print('Low MASK->',low_mask,'\nMatrix->',matrix)

        high_mask = matrix > high_value
        matrix = LoadImages.apply_mask(matrix, high_mask, high_value)

        return matrix

    def simplest_cb(img, percent):
        assert img.shape[2] == 3       #the three colors (red,blue,green)
        ...
        for channel in channels: 
            assert len(channel.shape) == 2    #ensures that the channel is a 2D array
            # find the low and high precentile values (based on the input percentile)
            height, width = channel.shape
            vec_size = width * height
            flat = channel.reshape(vec_size)         #reshape it into a 1D array
            # print('vec=',vec_size,'\nFlat=',flat)
            assert len(flat.shape) == 1

            flat = np.sort(flat)   #This line sorts the flattened array in ascending order. Sorting is necessary to determine the low and high percentile values accurately.
            ...
            thresholded = LoadImages.apply_threshold(channel, low_val, high_val)
            # scale the channel
...
class LoadStreams:
    ...
    def __next__(self):
        ...
        # Letterbox
        img0 = self.imgs.copy()                          #----------------------------------------------------------------------------
        cv2.imshow('ori',img0[0])
        img0[0] = LoadImages.simplest_cb(img0[0], 1)  
        cv2.imshow('dehazed',img0[0])        
        # cv2.waitKey(5000)
